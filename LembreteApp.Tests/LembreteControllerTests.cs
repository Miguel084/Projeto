...
namespace LembreteApp.Tests
{
    public class LembretesControllerTests
    {
        ...
        public void PutLembrete_DeveRetornarNotFoundQuandoLembreteNaoEncontrado(Lembrete lembrete)
        {
            // Arrange
            _lembreteServiceMock.Setup(service => service.Get("1")).Returns(lembrete);

            // Act
            var result = _controller.Update("1", new Lembrete { Id = "1", Nome = "Lembrete 1", Data = DateTime.Now.AddDays(1) });
        ...
        [Fact]
        [MemberData(nameof(LembreteData))]
        public void PutLembrete_DeveRetornarNoContentQuandoLembreteEncontrado()
        {
            // Arrange
            var lembrete = new Lembrete { Id = "1", Nome = "Lembrete 1", Data = DateTime.Now.AddDays(1) };
            _lembreteServiceMock.Setup(service => service.Get("1")).Returns(lembrete);
